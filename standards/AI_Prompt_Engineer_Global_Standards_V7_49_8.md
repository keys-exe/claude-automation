...
**Version 7.49.8 — supersedes all prior versions.** *(the avatar sheet is locked: `SHEET-GRID` layout, GPT Image 2.5 Sunburst as its route on a measured win, heavy-male-features skin confirmed)*
...
1. **Photorealistic (Mode 1)** — Fully realistic camera footage or photography. **Shot on iPhone 17 Pro Max, always** *(moved from the 13 Pro Max at V7.49.6)*. Real skin texture, natural camera physics, real environments, believable human motion, no AI look. The camera is a lock, not a default (§22): any build whose reference or inspiration reads as realistic is Mode 1 on the iPhone 13 Pro Max, and it is never negotiated per build, per act, or per beat.
2. **Semi-Realistic Adult 3D (Mode 2)** — Cinematic high-end animated feature look for adult characters. Grounded proportions, detailed stylized materials, expressive but believable faces, mature cinematic lighting. A premium animated feature for grown audiences — not a kids' cartoon, not a toy render, not live-action.
3. **Full Disney/Pixar (Mode 3)** — Classic mainstream animated-feature look for general/family audiences and child or all-ages characters. Warmer and more stylized than Mode 2 — bigger emotive eyes, softer exaggerated proportions, bright storybook-cinematic palettes.

4. **Stylized 3D Social (Mode 4)** — Caricatured adult-proportioned 3D for short-form social and comedic direct response. Knowing rather than sincere: sclera-dominant eyes cut off-axis, heavy expressive brows, glossy skin, aggressive shallow depth of field. **Its defining feature is a controlled register split inside the frame** — stylized character, near-photoreal props and product (§24E).

5. **Stop-Motion Clay (Mode 5)** — Handmade plasticine puppets on wire armatures, photographed frame by frame on a real miniature set. **It is the only stylized mode that is a capture rather than a render**, and everything that makes it read comes from that: thumbprints in the clay, visible seams over the armature, the surface simmering between frames, stepped movement at twelve poses a second, and real macro depth of field on a small object (§24F).

The five are **not interchangeable.** Mode 2 = mature, restrained, adult-proportioned. Mode 3 = expressive, family-friendly, sincere. Mode 4 = caricatured, adult, comedic, knowing. Mode 5 = handmade, tactile, warm, and visibly physical. Pick on audience, character type and emotional register — not "cartoon vs. not cartoon."

---

## 2. Mode Selection Rules

- If the user states the mode, follow it.
- If a mode is established in the project, keep it locked.
- If no mode is established, ask before creating any prompts.
- No drift between modes mid-project.
- No real-camera or photographic language inside animated modes unless requested.
- No adult-restrained stylization language in a Mode 3 project; no bright storybook language in a Mode 2 project.

**Hybrid register exception:** builds may mix registers by beat — mechanism beats in the locked mechanism register (§12A), lifestyle beats in Mode 1, hero product beats in Mode 1 photoreal. Allowed when established, but **each beat must be internally pure.** Never blend two registers inside one shot.

**Mode 4 is the single exception, and it is a spec rather than a licence.** §24E requires a controlled split *inside* the frame — stylized character against near-photoreal props and environment surfaces. The split runs on three named tiers (`M4-SPLIT`) and nothing else. It does not authorise free blending anywhere else in the document, and it does not travel to Modes 1–3.
...
**Image:** Nano Banana Pro, Nano Banana 2, GPT Image 2.5 (GPT Image 2 as fallback), Seedream 4.5
...
### GPT Image 2.5 — the challenger route on wordmark beats *(new V7.49.7 — vendor-documented and catalogue-confirmed, not yet measured in this pipeline)*

GPT Image 2 (April 2026) was superseded by **GPT Image 2.5** on 8 September 2026, which ships as two variants: **Flare**, the default, matching GPT Image 2 quality at up to half the latency, and **Sunburst**, the precision tier. Both generations are in the Higgsfield catalogue — `gpt_image_2` and `gpt_image_2_5` — with 9:16 among the aspect ratios and reference images accepted (`gpt_image_2_5` takes them under the role `image_references`; `gpt_image_2` under `image`). **The arsenal entry is 2.5; 2 is the fallback where 2.5 is unavailable or refuses a beat.**
...
| Candid B-roll face seeds (§22T) | `nano_banana_pro` stays | GPT Image on the same seed is the §22T A/B. A model that cleans the file is off the Mode 1 route regardless of skin quality |
...
| Volume B-roll, no type | `nano_banana_2` stays | Nothing to gain |
...
**Modes 2 and 3:** the same contact logic applies, stylized. The band still dents the form, the product still has a contact shadow. Only the skin micro-detail drops.
...
| **Sculpted, in-register** — the build runs Mode 5 | §24F's `M5-DIAGRAM`. The mechanism is built as a clay object and photographed on the same set, never cut to a render |
...
**Density D is a deliberate Mode 1 register break** — a physical model on a real surface, handheld. It drops the base world entirely, takes the full §22A capture block **and** the §22B R1 arc. Densities A–C take neither: they are renders, not captures, and are exempt from §22A.
...
A post graphic inherits the register of the picture underneath it. On Mode 1 the sanctioned kit above is correct as written: clean vector, no outline stroke, no bevel. **On Mode 5 that same clean vector is a register break**, and the layer is clay-styled instead — `M5-TYPE`. The rule generalises: before adding any mark, ask what it would have been made of in the world it is landing on.
...
| 2 | **Absorb script, product, Product Sheet** | DET | The `.md` + `.py` pair, created where absent. §43A claims pass — every figure tiered before anything builds against it. **The phrase inventory is built here** (§27B): a mechanical pass over the script as written. **Every lock resolved here** — mode and register, camera, format, tools and model strings, mechanism claim, declared side |
...
| 4 | **Location maps** | DET | The §30C Location Derivation Pass over the step-2 phrase inventory. Five-part Location Sheets, §22A lighting profiles, plates rendered for **PLATED locations only** — never for INCIDENTAL or **TRAVERSED** ones (§30C 1a). Four set-level checks run. **The location set closes here.** **Sent, then straight on** |
...
A phone file is a *found* image and a cinema capture is a *made* one. Shallow anamorphic depth of field, graded contrast, controlled falloff and clean highlights are all decisions someone paid for, and a viewer reads them instantly as advertising. Smart HDR flattening the shadows, a window blowing out because nobody lit it, 26mm edge softening, noise where the room went dark — nobody chose any of that, and it is the absence of choices that reads as true. On direct response this is the stronger asset, not the compromise.
...
**Vocabulary ban (mirrors §5):** never write *beautiful, flawless, perfect skin, glowing, radiant, youthful complexion* in a Mode 1 human prompt. Each is a trained association with retouched stock photography and pulls the render toward the beauty-filter failure. Skin is described only through `SKIN-A`'s positive imperfections.
...
**Mode 5 takes R6 and nothing else** — a bolted tabletop rig moved a fraction between exposures. It is a capture, not a render, but it is not a handheld capture: drift, sway and focus hunt are as false there as on a render rig.
...
**Modes 2, 3 and 4 take the render rigs — RV, RV-FAST or R4 — never a handheld rig.** There is no operator and no phone, so breath sway, gait bounce, arm-fatigue drift and focus hunt are all false. They are **not exempt from the four-part arc**, and they take no part of §22A. Same carve-out logic as mechanism densities A–C: the camera moves, the capture register does not apply.
...
## 23. Mode 2: Semi-Realistic Adult 3D Standard

**Character design** — grounded adult proportions, believable facial anatomy, expressive but not oversized eyes, subtle stylized skin texture, soft subsurface scattering, detailed hands, natural hair strand clusters, mature design, on-model across all scenes.

**Materials/rendering** — PBR-informed shading, soft global illumination, cinematic shadows, realistic material response, detailed fabric, hair detail, feature-film rendering quality. No flat cel-shading, no toy-like gloss.

**Environment** — detailed cinematic environments, atmospheric depth, mature lighting, realistic bokeh, rich production design, sophisticated palette.

**Motion** — grounded movement, natural weight shifts, believable facial expression, cloth sway, hair movement, subtle muscle movement, controlled gestures, cinematic camera. No exaggerated squash-and-stretch.

**Negatives — `NEG-M2`.**

**§24A, §24B, §24C and §24D govern Mode 2 as well**, at different values: realistic iris proportion, 7–7.5 heads, subtler anticipation, and a restrained colour script. Read them there rather than duplicating them here.

---

## 24. Mode 3: Full Disney/Pixar Standard

**Character design** — expressive, appealing proportions with room for stylization: larger emotive eyes, softened rounded facial structure, exaggerated-but-charming expressions, classic rigging language. Child and all-ages proportions fully allowed. **Shape language and proportion per §24A.**

**Materials/rendering** — soft PBR-informed shading tuned for warmth, gentle global illumination, saturated but tasteful grading, classic feature-animation quality. **Lighting design per §24B.**

**Environment** — bright, rich, storybook-cinematic; emotionally legible set dressing. **Colour script per act, §11.**

**Eyes — §24C.** **Motion — §24D**, written as an arc, never as the adjective stack prior versions carried.

**Negatives — `NEG-M3`.**

**Quick check:** child, mascot, or warm/family/educational brand tone → Mode 3. Adult protagonist in a mature restrained cinematic register → Mode 2. Adult comedic short-form social → **Mode 4** (§24E).

### Register warning for direct response

**Mode 3 reads as content made for children.** On a product sold to adults that is a register mismatch which costs credibility before the script gets a word in — §19's point is that credibility lives in a lived-in environment, and a storybook palette is the opposite of one.

**Mode 3 earns its place on a mechanism or metaphor sequence inside a Mode 1 build** (§2 hybrid exception), where the stylization is doing explanatory work rather than carrying the pitch. As the register for a full DR ad to an adult buyer, push back and offer Mode 4 or Mode 2 instead.

---
## 24A. Shape Language and Proportion *(new — unverified, visual check)*

**Scope:** Modes 2, 3 and 4 character design. The biggest single lever in stylized character work, and absent from V6 entirely.

**This is §8's asymmetry rule in a different register.** Generators regress toward the mean, so whatever departs from the mean must be named explicitly or it is normalised out. In products that means symmetrised peaks and centred wordmarks. In characters it means generic cute-3D mush with no design in it.

**NORMATIVE — `M3-SHAPE` — see Appendix A.**

The thumbnail test is the one to keep: **if the silhouette is not readable filled solid black at thumbnail size, the design has no shape language and the beat will not survive a feed.**

### Proportion ladder

Head-to-body height is the single control determining how old a stylized character reads. **State it on the reference sheet (§19)** — leaving it unstated is how a character drifts age between beats.

| Reads as | Mode 3 | Mode 2 | Mode 4 |
|---|---|---|---|
| Small child (3–7) | 3–4 heads | not used | not used |
| Older child (8–12) | 4.5–5 heads | 6 heads | not used |
| Teen | 5.5–6 heads | 7 heads | 6 heads |
| Adult | 6–6.5 heads | 7–7.5 heads | **6–6.5 heads** |
| Elder | 5.5–6 heads, compressed posture | 7 heads, compressed posture | 6 heads, compressed posture |

---

## 24B. Stylized Lighting Design *(new — unverified, visual check)*

**Scope:** Modes 2, 3 and 4.

Mode 1 gets realism from naming **file artefacts** (§22A). Stylized modes get appeal from naming **lighting-design decisions**. Neither gets it from naming a mood — *"cinematic lighting"* is a room word, the same class as *"natural window light"* and overridden for the same reason.

**NORMATIVE — `M3-LIGHT` — see Appendix A.**

**The rim is doing the most work.** Uniform ambient with no separation is the flat, unlit, asset-store read that every stylized mode collapses into by default.

**Mode 4 substitutes `M4-BASE`**, which carries the same three sources plus an aggressive depth-of-field clause.

---

## 24C. Stylized Ocular Standard *(new — unverified, visual check)*

**Scope:** Modes 2, 3 and 4. §28E's four-part gaze arc still governs *where the gaze goes and when* — this adds the physical spec underneath it.

Stylized eyes are physically larger and occupy more of the frame, so every eye failure is magnified. **The dead-eye is almost always a missing catchlight**, which is a one-clause fix.

**NORMATIVE — `M3-EYES` — see Appendix A.**

**Mode 4 inverts the iris rule** — sclera-dominant, small dark iris, anchored off-axis. See §24E.

---

## 24D. Stylized Motion Arc *(new — unverified, visual check)*

**Scope:** Modes 2, 3 and 4. The stylized counterpart to §27A, and the last state-written section in the document.

**NORMATIVE — `M3-MOTION` — see Appendix A.**

**The moving hold is the stylized version of "never at rest at the cut."** It is the same clause §22B, §27A and §28E all depend on, stated in animation language — which is why it joins them on §37's never-trim list.

**Anticipation is the clause generators skip.** An action that begins without a small counter-move in the opposite direction reads as a pose swap, not a movement. Name it every time.

**Overlap never settles on the same frame as the body.** Hair and cloth stopping in sync with the torso is the clearest render tell in any stylized mode.

---

## 24E. Mode 4: Stylized 3D Social *(new — unverified, visual check)*

**A peer of Modes 1–3, not a child of Mode 3.** It carries a §24-family number only because V7 preserves every § number so corrections by ID keep working.

### What separates it from Mode 3

Five differences, and every one inverts a Mode 3 rule:

| | Mode 3 | **Mode 4** |
|---|---|---|
| Eyes | Iris-dominant, sclera minority | **Sclera-dominant, small dark iris, cut off-axis** |
| Skin | Matte, soft subsurface | **Glossy, visible specular on nose bridge and cheekbones** |
| Performance | Sincere, open, warm | **Knowing, sly, in on something** |
| Render purity | One register per shot | **Deliberate split — stylized character, photoreal props** |
| Depth of field | Sparingly, to separate subject from set | **Aggressive, round bokeh discs as a core look element** |

### The blocks

**`M4-BASE`** — render and lighting. Replaces `M3-LIGHT` in this mode.
**`M4-FACE`** — character design. Replaces the §24C iris rule.
**`M4-PERF`** — **the signature. This is the block that produces the look.**
**`M4-SPLIT`** — the controlled register split.
**`M4-ANIMAL`** — animals as characters.
**`NEG-M4`** — negatives. All in Appendix A.

§24A shape language, §24D motion and §11's colour script all apply unchanged.

### The performance lock is the register

Everything else on this list is render settings. **The thing that makes a beat read as Mode 4 is the face**: eyes cut off-axis, one brow higher, closed asymmetric smirk, head turned slightly against the direction of the glance. A human and an animal running that same expression read as the same world.

**It is a default, not the only face.** A build where every beat is the knowing smirk has no turn, fails §28A's no-turn test, and assembles as one long shot. `M4-PERF` sets the resting register; **§28A and §28E still govern the arc**, and direct-address beats override the off-axis anchor.

### The split is why this register is commercially worth having

`M4-SPLIT` means **the product renders near-photoreal inside a stylized world.** That is the thing animated ads normally cannot do — the product either goes cartoon and stops being the thing being sold, or the edit cuts to live footage and breaks the register.

Three tiers, and only three:

| Tier | Treatment |
|---|---|
| **Character** — skin, hair, clothing, animals | Fully stylized |
| **Held objects** — product, packaging, food | Near-photoreal materials at stylized scale on simplified forms |
| **Environment surfaces** — wood, tile, fabric, worktop | Photoreal materials on simplified forms |

**Nothing else splits.** Lighting is one system across all three tiers, or the split reads as a compositing error rather than a style.

### Small type garbles here exactly as in Mode 1

§17 applies with full force. Tags, labels, collar discs, packaging print and wordmarks all garble. **Blank in generation, added in CapCut** — including the product wordmark, unless the beat attaches the product reference image and passes the §5 first-frame check.

### Consistency is harder in this register, not easier

The caricature carries so much of the identity that a small drift in brow shape or jaw plane reads as a different person. **§19's composite reference sheet and §5's reference-image discipline matter more in Mode 4 than in Mode 1**, where skin texture and bone structure give the model far more to hold onto.

### Where it fits in a build

Mode 4 is a **hook and pattern-interrupt register**. It stops a feed and it survives a comedic angle that Mode 1 cannot carry. It is weaker on proof and testimonial, where §19's lived-in-environment credibility is the whole point and a stylized presenter has none of it.

**Position: Mode 4 for the hook and the mechanism, Mode 1 for proof and the close** — the §2 hybrid pattern, beat-pure throughout.

---

## 24F. Mode 5: Stop-Motion Clay *(new — unverified, visual check)*

**A peer of Modes 1–4.** It carries a §24-family number because V7 preserves every § number so corrections by ID keep working.

### What separates it from every other stylized mode

**Mode 5 is photographed, not rendered.** Modes 2, 3 and 4 are renders and are therefore exempt from §22A and restricted to render rigs. Mode 5 is a real camera pointed at a real object, so the capture logic returns — but at miniature scale and on a bolted-down rig, never handheld.

That single fact generates every rule below, and it is also the thing a generator will take away from you if it is not stated. Asked for "claymation" a generator returns **a 3D render of a clay-textured character**: smooth, symmetrical, evenly lit, moving continuously. That is the Mode 5 equivalent of plastic skin, and it is defeated the same way §8 defeats symmetry — by naming what the handmade object has that the render does not.
...
`M5-BASE` establishes that it is a photograph of a physical thing · `M5-MAT` is the handled surface and the single most important block in the mode · `M5-BOIL` is the frame-to-frame simmer and it is what the eye reads as "stop-motion" before anything else · `M5-SET` is the miniature world and the macro lens. `M5-FACE` covers character heads; `M5-MOTION` lives in I2V.

**Never trimmed: `M5-MAT`'s thumbprint clause and `M5-BOIL`.** A perfectly smooth clay character that holds still between frames is a render wearing a clay texture, whatever else the prompt says.

### Proportion and shape

§24A governs, at clay values: **4–5 heads tall**, oversized hands and feet because an armature needs mass to stand up, a heavy low centre of gravity, simple readable silhouettes with no fine extremities. The thumbnail test applies unchanged.

### Motion — §24D is replaced, not adjusted

`M5-MOTION` supersedes `M3-MOTION` in this mode. The moving-hold rule inverts: a stop-motion hold is a genuinely **held** position with only the boil moving, and that is correct rather than a failure. Anticipation and follow-through still apply because the armature has real weight. **`no motion blur` is absolute** — every frame is a still photograph of a stationary object, and a smear is the clearest tell that a generator has fallen back to rendering.
...
Mouths are **replacement shapes pressed on per sound**, not a deforming face. §28F's jaw-carried hierarchy does not apply and is replaced by the mouth-shape clause in `M5-FACE`; the closure-word rule survives, because a closed-lip replacement shape on the named word is exactly as checkable as a real one. §28B gesture landings work normally — an armature is built to hold a pose — but finger counts past two are worse here than anywhere, since clay fingers fuse.

### Where it fits in a build

Mode 5 buys enormous warmth and pattern-interrupt value, and it carries the same §24 register warning as Mode 3: handmade reads as charming, which helps a story act and costs credibility on a proof act. **Position: Mode 5 for the hook, the story and the mechanism metaphor, Mode 1 for proof and the close** — the §2 hybrid pattern, beat-pure throughout.
...
The blocks above say what Mode 5 looks like. Everything below is what a build actually needs to ship one, and it is where the mode differs most from the others in practice.
...
**Generate the movement, then posterize time to 12fps in CapCut.** The step is mechanical, exact, free, and identical across every clip in the build — which is what a real stop-motion production has and a per-beat generated approximation never will. `M5-MOTION` stays in the prompt for the *character* of the movement — real armature weight, settling, no smear — and the frame rate is imposed afterwards.

#### The generator's worst artefact is this mode's signature

Frame-to-frame flicker and surface instability — the thing every other register fights, and the reason §30 exists — **is the boil.** In Mode 5 it is free and it is correct, so do not suppress it: no stabilisation, no denoise, no frame-blending, no temporal smoothing anywhere in the pipeline. Mode 5 is the one register where AI's native inconsistency is an asset, and it is the reason a difficult look is unusually cheap to produce.

#### The product is never clay

The §24E split, applied here and non-negotiable: **the puppet is sculpted, the world is built, and the product is real.** A plasticine version of the product stops being the product and the ad stops selling it. `M5-PROD` states it, and the contrast is a feature — the one hard-edged, true-coloured, properly finished object in a handmade world draws the eye exactly where the beat wants it.
...
A clay puppet is easier to hold than a human face and it drifts differently: sculpt stays roughly put, **colour and seam position wander**. The reference sheet is a photograph of the puppet on the bench, five views, one generation — and it locks four things the §19 sheet has no field for: the exact clay colours and how they were mixed, seam positions, eye type and spacing, and height relative to a set piece. `M5-PUPPET` opens every beat that puppet appears in, with its reference attached. Image plus names, exactly as everywhere else in this document.

#### Dialogue — Mode 5 speaks, but never front-on *(amended V7.45)*

**The earlier rule said Mode 5 does not do dialogue at all. That was too broad and it is withdrawn.** Clay characters talking to each other is one of the mode's strongest assets — a scene between two puppets carries more warmth than any talking head in the document — and banning it removes the reason to reach for the register in the first place.

The real constraint is narrower and it is about the mouth only. Lip-sync models generate a deforming mouth on a continuous face; Mode 5 mouths are **replacement shapes**, a discrete swap per sound. Asked for both, a generator returns a clay face with a rubber mouth, which is the loudest break available in the mode.

**So: characters may speak freely, and every speaking beat is framed off-mouth.** `M5-SPEAK` carries it — from behind, three-quarter rear, profile with the far side turned away, over the listener's shoulder, or cut to the hands and what they are holding. The head still moves with the speech and the body still performs; only the front-on mouth is excluded.
...
- **§28F does not apply in this mode.** The jaw-carried hierarchy, the amplitude clause and the closure landing all assume a visible mouth. `M5-SPEAK` replaces the whole section.
...
Build-level position is unchanged: **Mode 5 runs narrated (§3A)**, with character dialogue placed inside that as off-mouth beats rather than as talking heads.

#### The mechanism in clay — a fifth register *(V7.45)*

§12A offers four mechanism registers: anatomical, material cutaway, comparative demonstration, or none. **Mode 5 needs a fifth, because none of the four survive the cut into a clay world.** Dropping a near-black medical-broadcast render into a handmade set is exactly the register break §2 forbids, and it costs more credibility than the diagram buys.

**The sculpted diagram: the mechanism is built as an object and photographed on the same set.** `M5-DIAGRAM`. A plasticine leg in relief on a card panel, propped at the character's shoulder or floating beside them, lit by the same lamps and carrying the same thumbprints as everything else in frame.
...
- **`ANAT-NEG`'s arrow ban inverts.** Arrows are banned in the render registers because generated overlay graphics garble. **A sculpted arrow is a prop, not an overlay** — it has thickness, thumbprints and a cast shadow, and it renders as reliably as any other object on the set. In Mode 5 force arrows, rings and marks are permitted *as clay objects*. They remain banned as flat graphics.

**Type on the diagram is still post** (§17), and in this mode the post layer has to match the world: `M5-TYPE`. Clean vector lettering over a clay diagram breaks the register as badly as a smooth puppet does.
...
Mode 5 sits close to the B-roll column of §37: no `dialogue`, no `delivery`, and the mode blocks all live in T2I under §6's relocation. I2V carries `INHERIT-SUBJ`, `M5-MOTION`, `M5-BOIL`, `RIG-R6` and `NEG-M5` — roughly 900 characters, comfortably inside the ceiling.

#### Assembly order, per beat

`M5-BASE` → `M5-MAT` → `M5-SET` → `M5-SCALE` → `M5-LIGHT` → `M5-PUPPET` if a character appears, reference attached → `M5-PROD` if the product appears, reference attached → `M5-FACE` if a head is in frame → negatives carrying `NEG-M5`. Then I2V: `INHERIT-SUBJ` → `M5-MOTION` → `M5-BOIL` → `RIG-R6` → `NEG-M5`. Then the CapCut line: posterize to 12fps, no stabilisation, no denoise, no frame-blend.

**NORMATIVE — `M5-BASE`, `M5-MAT`, `M5-BOIL`, `M5-SET`, `M5-SCALE`, `M5-LIGHT`, `M5-PUPPET`, `M5-PROD`, `M5-FACE`, `M5-SPEAK`, `M5-DIAGRAM`, `M5-TYPE`, `M5-MOTION`, `M5-CAPCUT`, `RIG-R6`, `NEG-M5` — see Appendix A.**
...
| Modes 2–4 | `HOLD-C` | `NEG-WARP-C` |
| **Mode 5** | `HOLD-C` only | `NEG-WARP-C` **minus the flicker clause** |
| **Break beat (§27E)** | `HOLD-BREAK` **instead of** `HOLD-C` | `NEG-WARP-B` **instead of** `NEG-WARP-C`, plus `NEG-BREAK` |

**The Mode 5 carve-out is absolute.** An anti-flicker clause destroys the boil, which is the mode's signature and the reason it is cheap to produce (§24F).
...
| **S3** | No two profiles alike — two rooms sharing a palette and a blow point measure as no cut (§15) |
...
4. **The attachment set:** scene plate (§30C, PLATED locations only) + the subject's reference sheet (Part 1, if recurring) + product reference (§5, if the product appears) — and every referenced object also named in prose. Image + names, for every reference on the call. On a TRAVERSED location nothing is attached for the place; the profile, `GEO-LINE` and carried landmark do the work.
...
- **Mode 5:** `style` carries `M5-BASE` + `M5-MAT` + `M5-SET`, `motion` carries `M5-MOTION` + `M5-BOIL`, `camera.movement` takes `RIG-R6`, `negatives` take `NEG-M5`. §22A, `INHERIT-CAP` and every handheld rig are absent
- `style`: **§15A three-part surface clause on object beats** (§12 grade otherwise). On any beat in a plated location, `SCENE-REF` opens the scene description in the T2I and `NEG-SCENE` merges into `negatives`; loose props carry their `SCENE-STATE` lines (§30C). **In Modes 2–4, the mode's base block plus `M3-SHAPE`** — or `M4-BASE` + `M4-FACE` + `M4-SPLIT` in Mode 4
...
- `negatives`: `NEG-WARP-C` (§27D, first) + `NEG-CAM-TH` + selected `NEG-HAND` clauses + `NEG-CONT` + `NEG-AUD` + selected `NEG-MOUTH` clauses (§28F) + selected `NEG-PACE` clauses (§28G) + selected `NEG-PHYS` clauses + `NEG-HELD` if applicable + `NEG-M1` — or `NEG-M2` / `NEG-M3` / `NEG-M4` in a stylized mode
- **Modes 2–4:** `motion` also carries `M3-MOTION`; `camera.movement` takes a render rig (§22B); `style` carries the mode blocks; §22A and `INHERIT-CAP` do not apply
...
**Modes 2–4:** no fifth column needed. Under §6's relocation the mode blocks — `M3-SHAPE`, `M3-LIGHT` or `M4-BASE`, `M4-FACE`, `M4-SPLIT` — all live in **T2I at full length**, because every one of them is a property of the frame. I2V carries `INHERIT-SUBJ`, `M3-MOTION`, the render-rig arc and the negatives. A stylized beat with dialogue then sits close to the TH column; without dialogue, close to B-roll. **§22A and `INHERIT-CAP` are absent in both cases** — a render is not a capture.
...
**10. Stylized register selection → Mode 4 for adult comedic short-form, Mode 2 for mature adult narrative, Mode 3 only where the audience or the character is a child.** Mode 3 on an adult-buyer direct-response build is a register mismatch and gets pushed back on (§24).
...
**14. Stylized camera → render rigs only (RV, RV-FAST, R4).** Never handheld. Not exempt from the §22B arc; fully exempt from §22A (§22B).
...
**19. Image model → `nano_banana_pro` on any beat carrying a readable wordmark, `nano_banana_2` on everything else** (§4); **`gpt_image_2_5` Sunburst is the sanctioned challenger on wordmark beats** — `quality: high`, `resolution: 2k`, never the catalogue defaults — and takes the route only on a measured win (§4). In a reference-image pipeline there is no Element carve-out — the wordmark routes to Pro. **Resolution is 2k on every beat type, with no exceptions** (§4, V7.48).
...
**31. Camera → iPhone 17 Pro Max on every Mode 1 beat, locked.** A realistic reference selects Mode 1 and this camera automatically. Another camera only on explicit per-build instruction, recorded as an override. The capture block and the Location Profile are both mandatory — the file and the light are separate and neither substitutes for the other (§22, §22A, §42).
...
**47. Model verification → read the logged model on every completed job (§5).** The string passed is not evidence of the model run. Model-critical work runs in the platform interface until the routing is fixed.

**48. Stop-motion clay → Mode 5, photographed not rendered (§24F).** `M5-BASE` + `M5-MAT` + `M5-SET` in T2I, `M5-MOTION` + `M5-BOIL` in I2V, `RIG-R6` for the camera, `NEG-M5` in negatives. The surface is handled — thumbprints, seams over the armature, embedded dust — the frames never match each other, movement is stepped on twos, and there is no motion blur anywhere. A smooth symmetrical clay character that holds perfectly still is a render in costume.

**49. Mode 5 production → the step is post, the boil is free, the product is real (§24F).** Frame rate is posterized to 12fps in CapCut, never asked for in the prompt; stabilisation, denoise and frame-blending are banned because generator flicker IS the boil; the product is the real object on the set and is never sculpted; Mode 5 builds run narrated (§3A) with character dialogue placed inside as off-mouth beats.

**50. Mode 5 dialogue and mechanism (§24F).** Clay characters speak, and every speaking beat is framed off-mouth — `M5-SPEAK` replaces §28F in this mode, and the audio is TTS over the beat. The mechanism is **sculpted in-register** (`M5-DIAGRAM`), never cut to a §12A render; force arrows and marks are permitted **as clay objects** and remain banned as flat graphics. Post type and graphics are clay-styled (`M5-TYPE`) — a clean vector over a clay world is the same error as a smooth puppet.
...
**52. Structural integrity → declared positively on every I2V, all modes (§27D).** Same form, same proportion, same count, first frame to last; one small movement completing inside the clip; nothing leaving frame and returning. Negatives are the backstop, never the mechanism. Mode 5 drops the flicker clause — it destroys the boil.
...
**`RIG-R6`** — stop-motion tabletop rig, Mode 5 only. *(379)*
```
...
**`CAP-C`** — compressed capture, daylight. The production short form of `CAP-A` where the T2I is dense; `CAP-A` remains the full form and the never-trim reference. (262) *(262)*
```
Capture is a phone camera file, not a lit scene: Smart HDR lifting the shadows flat, slight compression softness at the frame edges from a 26mm lens, natural colour temperature straight off the phone. No colour grading, no retouched skin, no shaped or lit light.
```
...
**`NEG-WARP-C`** — universal. **Never trimmed. Mode 5 drops the flicker clause.** *(228)*
```
...
## Stylized registers — Modes 2, 3, 4

**`M3-SHAPE`** — shape language. Modes 2, 3, 4. *(429)*
```
...
**`M3-LIGHT`** — stylized lighting design. Modes 2 and 3. **Mode 4 substitutes `M4-BASE`.** *(504)*
```
...
**`M3-EYES`** — stylized ocular spec. Modes 2 and 3. **Mode 4 inverts the iris clause — see `M4-FACE`.** *(346)*
```
...
**`M3-MOTION`** — stylized motion arc. Modes 2, 3, 4. **Never trimmed** (§37). *(404)*
```
...
**`M4-BASE`** — Mode 4 render and lighting. *(468)*
```
High-end stylized 3D animated feature render, feature-quality CGI. Warm practical domestic lighting — lamp light, window light, kitchen light — with a strong warm key and a cooler fill. Visible rim light separating the character from the background. Aggressive shallow depth of field: the background falls to soft defocus within a foot of the subject, with round bokeh discs on any practical light in frame. Rich warm grade, deep saturated shadows, glowing highlights.
```

**`M4-FACE`** — Mode 4 character design. *(490)*
```
Caricatured adult proportions, roughly six heads tall, sculpted cheek and jaw planes. Heavy dark eyebrows, thick and sharply shaped, carrying most of the expression. Eyes large but sclera-dominant — a small-to-mid dark iris sitting in a wide visible white, never the iris-filled Disney eye. Skin glossy with visible specular on the nose bridge, forehead and cheekbones, never matte. Hair in sculpted clumped masses with a soft specular sheen along the top surface, never individual strands.
```

**`M4-PERF`** — **the Mode 4 signature.** Sets the resting register only; §28A and §28E still govern the arc. *(351)*
```
Default expression is knowing, not sincere: eyes cut off-axis to one side rather than to the lens, one eyebrow raised higher than the other, mouth closed in a small asymmetric smirk pulled to one side. Head turned slightly away from the direction the eyes are looking, so the glance reads sidelong rather than direct. The character is in on something.
```

**`M4-SPLIT`** — controlled register split. **The one authorised intra-shot blend in the document** (§2). *(385)*
```
Controlled register split inside the frame. Character, skin, hair and clothing are stylized. Held objects, food, packaging and product carry near-photoreal materials — real surface texture, real translucency, real specular response — at stylized scale on simplified forms. Environment surfaces are photoreal materials on simplified forms: real wood grain, real tile, real fabric weave.
```

**`M4-ANIMAL`** — animals as Mode 4 characters. *(300)*
```
Accurate species anatomy, coat and proportion, with two human features added: sharply shaped expressive eyebrows set above the eye, and a mouth capable of a closed asymmetric smirk. Eyes placed forward enough to hold a sidelong glance. Never a full anthropomorphic body, never bipedal, never clothed.
```
...
## Mode 5 — stop-motion clay

**`M5-BASE`** — Mode 5 core. Opens every Mode 5 T2I. *(347)*
```
...
**`M5-MAT`** — the handled surface. **The block that produces the mode. Never trimmed.** *(650)*
```
...
**`M5-BOIL`** — the frame-to-frame simmer. I2V, and stated in T2I as surface irregularity. **Never trimmed.** *(378)*
```
...
**`M5-SET`** — miniature set and macro lens. *(545)*
```
...
**`M5-FACE`** — clay character heads. Replaces §24C's iris rule and §28F's jaw hierarchy in this mode. *(373)*
```
...
**`M5-MOTION`** — I2V `motion`. Supersedes `M3-MOTION` in Mode 5. *(556)*
```
...
**`M5-SCALE`** — miniature scale cues. *(349)*
```
...
**`M5-LIGHT`** — tabletop practical lighting. Replaces `M3-LIGHT` in Mode 5. *(445)*
```
...
**`M5-PUPPET`** — puppet consistency. Opens every beat a locked puppet appears in, reference attached. Substitute the named sculpt asymmetries. *(357)*
```
...
**`M5-PROD`** — the product in a clay world. **Never trimmed.** *(409)*
```
...
**`M5-SPEAK`** — off-mouth dialogue coverage. Replaces §28F entirely in Mode 5. *(434)*
```
...
**`M5-DIAGRAM`** — the sculpted mechanism diagram. The Mode 5 mechanism register (§12A). *(568)*
```
...
**`M5-TYPE`** — clay-styled post layer. CapCut instruction, never a prompt. *(360)*
```
Mode 5 type and graphics are clay-styled in the edit: lettering set to read as rolled or pressed plasticine with uneven strokes and a slight drop shadow, marks and rings drawn with a hand-wobbled edge rather than a vector-true one, everything on twos with the rest of the picture. Never a clean sans-serif, never a flat vector arrow, never a UI-styled callout.
```

**`M5-CAPCUT`** — the standing edit lines for every Mode 5 build (§40). *(255)*
```
Standing Mode 5 lines: posterize time to twelve frames a second across every Mode 5 clip · do not stabilise, do not denoise, do not frame-blend — all three destroy the boil · no motion blur added at any point · retime before any speed change, never after.
```
...
**`NEG-M2`** *(351)*
```
no toy-like character, no childish cartoon, no oversized head, no exaggerated eyes, no flat cel-shading, no glossy plastic skin, no rubbery face, no squash-and-stretch, no juvenile color palette, no storybook simplification, no full live-action photorealism, no off-model character drift, no inconsistent proportions, no warped hands, no deformed face
```

**`NEG-M3`** — rebuilt in V7 from 11 clauses to 25. *(668)*
```
no photorealistic skin, no visible skin pores, no live-action texture, no adult-restrained proportions, no horror-uncanny facial anatomy, no dead eyes, no missing catchlight, no flat lifeless expression, no uniform ambient lighting, no unlit flat render, no mobile game 3D, no asset-store character, no generic rounded character with no shape language, no plastic toy sheen, no perfectly symmetrical face, no off-model character drift, no inconsistent proportions across shots, no muddy or desaturated palette, no warped hands, no deformed face, no frozen pose, no true stillness, no rigid cloth, no settled hair, no overly mature or gritty lighting mismatched to tone
```

**`NEG-M4`** — Mode 4. *(594)*
```
no photorealistic human skin, no visible skin pores, no matte skin, no iris-dominant Disney eyes, no oversized childlike eyes, no live-action texture on the character, no plastic toy sheen, no flat uniform lighting, no deep focus, no dead eyes, no missing catchlight, no text, no letters, no numbers, no labels on props, no readable wordmark, no logo, no perfectly symmetrical face, no off-model character drift, no inconsistent proportions across shots, no cold grey grade, no desaturated palette, no warped hands, no deformed face, no frozen pose, no true stillness, no stock open-mouth smile
```
...
| Mode 4 split runs three tiers only | Lighting stays one system across all three, or the split reads as a compositing error rather than a style |
| Mode 4 smirk is a default, not the only face | Every beat on one expression has no turn, fails §28A, and assembles as one long shot |
| Mode 3 pushed back on for adult DR | It reads as content made for children; §19 credibility comes from a lived-in environment, not a storybook palette |
...
| Model actually run | logged model on completed job vs passed string | exact match | AUTO | Re-submit with explicit string; flag alias to phrasing table |
...
One row per beat: `beat_id · phrase_ids[] · t2i_prompt_path · t2i_job_id · t2i_status · t2i_qa (pass/fail per check) · i2v_prompt_path · i2v_job_id · i2v_status · i2v_qa · retries[{class, action, ts}] · attachments[{role, media_or_job_id}] · delivered (bool) · reissue_flag (standard §, reason)`. Plus build-level: `plates{location→job_id}` · `subjects{S-id→job_id}` · `story_days{day→outfit_row}` · `capture_events{event_id→{story_day, location_id, beats[]}}` · `declared{side, mechanism_claim, format, mode}` · `version_built_against`. The ledger is the source for §34 global scans, reissue passes, resume-after-interruption, and the Open Decisions counts — **computed, never hand-maintained**.
...
9. **§24A–§24E — visual check, one generation each.** No instrument needed; they either match the reference or they do not.
   - **§24E first**, against the supplied Mode 4 references. `M4-PERF` is the block to judge — if the off-axis glance and the asymmetric smirk do not land, nothing else in the mode matters.
   - **§24A** — generate one character, then check the silhouette filled solid black at thumbnail size.
   - **§24B / §24C** — one beat with the three-source block and the catchlight clause against one without. Judge rim separation and dead-eye.
   - **§24D** — the last-frame delta from decision 1, pointed at a render. Tests the moving hold directly.
...
**GPT Image 2.5 — two first-use tests, in order** *(V7.49.7)*: (1) one held-product beat, `nano_banana_pro` vs `gpt_image_2_5` Sunburst at `high`/2k, same prompt and reference, scored on wordmark legibility, the uneven peaks and the register — decides the wordmark route; (2) the PF-BR-01 medium-close seed on `gpt_image_2_5`, scored against the confirmed Nano Banana frame on the four §22T light items and the skin — decides whether the model is on the Mode 1 route at all. Logged model read on every job. Visual-check, not counted against the §45 cap.
...
**Visual-check, not counted** — §24A, §24B, §24C, §24D, §24E, the two unverified Location Profiles (with the skin-under-overcast check), the §30B register gate, the §9A-P inner-face read, plus the visual checks recorded above (§12B, §27C, the ANAT-STRESS pair, the §30C scene hold, and §30E's subject-plate and axis reads). They sit here until someone generates one and looks — the count is whatever the list says, computed, never hand-maintained.

---

# CHANGELOG — V7.49.7 → V7.49.8 *(cut authorised)*

| § | Change |
|---|---|
| 19 | **Sheet locked.** `SHEET-GRID` is the layout — stated in fractions, one floor line, one head line, figures centred, true 90° profiles, full-length back view, head-to-collarbones close-up. Route: `gpt_image_2_5` Sunburst, `high`, 2k, on a measured win across two face types; GPT Image 2 the fallback; Nano Banana Pro no longer routed for sheets. Sheets are centred, beats are not |
| 19A | Scope note updated: heavy male features and weathered outdoor skin confirmed; darker skin tones still open |
| 4 | GPT Image routing table: the avatar-sheet row moves from candidate to measured |
| A | `SHEET-GRID`, `NEG-GRID` |
| D, E1 | Three rationale rows; the sheet QA row gains the grid items and the logged-parameter check |
| Open Decisions | §19 entry updated |

**Origin:** the fifth avatar sheet of the day. Stating the grid in fractions produced a sheet the previous four had approximated — heads on one line, feet on one line, profiles at ninety degrees, nothing cut off — on a model that follows layout instructions, and it did so on the heavier face type the skin stack had never been tested against.

---

# CHANGELOG — V7.49.6 → V7.49.7 *(cut authorised)*

| § | Change |
|---|---|
| 4 | **GPT Image 2.5 enters the image stack** — new subsection: Flare and Sunburst, the Higgsfield ids, the two catalogue traps (`quality` and `resolution` both default low), the per-beat-class routing with it as the sanctioned challenger on wordmark beats, the reasoning-model risk and its first-frame item, and the two ordered first-use tests. GPT Image 2 becomes the fallback |
| 44.19 | Default restated with the challenger |
| E7 | GPT Image 2.5 call template, marked unverified |
| D | Three rows |
| Open Decisions | The two first-use tests |

**Origin:** a request to add GPT Image 2 to the arsenal. A catalogue lookup found 2.5 already listed with the vendor's text-rendering and reference-fidelity claims, which map onto two named failures in this document — so it enters where those failures live, and nowhere else until measured.
